			...
			sjObj.defaults = {
				...
				buildHTml: function() {
					...
					if(sjObj.opt.Format == "dd-mm-yyyy") {
						if(sjObj.opt.Day) $(sjObj.opt.timeElm.find('.df-wrap')[0]).find('tr').append("<td>" + sjObj.opt.buildArrStr(sjObj.opt.ddArr, sjObj.opt.dayText, "dd", "日") + "</td>");
						if(sjObj.opt.Month) {
						...
						if(sjObj.opt.Year) {
							var eml = sjObj.opt.df_wrap();
							$(eml).find('tr').append("<td>" + sjObj.opt.buildArrStr(sjObj.opt.yyArr, sjObj.opt.yearText, "yyyy", "年") + "</td>");
							main.append(eml);;
						}
					} else if(sjObj.opt.Format == "mm-dd-yyyy") {
						if(sjObj.opt.Month) $(sjObj.opt.timeElm.find('.df-wrap')[0]).find('tr').append("<td>" + sjObj.opt.buildArrStr(sjObj.opt.mmArr, sjObj.opt.monthText, "mm", "月") + "</td>");
						if(sjObj.opt.Day) {
							var eml = sjObj.opt.df_wrap();
